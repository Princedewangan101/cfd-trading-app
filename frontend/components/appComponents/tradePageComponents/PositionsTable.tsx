...
const formatPnl = (value: string | number | null | undefined) => {
    const pnl = value === null || value === undefined || value === "" ? 0 : Number(value);
    const sign = pnl > 0 ? "+" : "";
...
const SkeletonRow = () => (
    <tr className="border-b border-zinc-900 last:border-b-0">
        ...
        <td className="px-3 py-3 text-right"><div className="skeleton ml-auto h-4 w-10 rounded" /></td>
        <td className="px-3 py-3 text-right"><div className="skeleton ml-auto h-4 w-8 rounded" /></td>
        ...
        <td className="px-3 py-3 text-right"><div className="skeleton ml-auto h-4 w-12 rounded" /></td>
        <td className="px-3 py-3 text-right"><div className="skeleton ml-auto h-4 w-12 rounded" /></td>
        <td className="px-3 py-3 text-right"><div className="skeleton ml-auto h-4 w-16 rounded" /></td>
        <td className="px-3 py-3 text-right"><div className="skeleton ml-auto h-4 w-12 rounded" /></td>
                    ...
                    <tbody>
                        {isLoading
                            ...
                            : empty
                                ? (
                                    <tr className="border-b border-zinc-900 last:border-b-0">
                                        <td colSpan={13} className="py-2 pl-1 pr-3">
                                ...
                                : filteredOrders.map((order) => {
                                    const pnl = order.pnl === null || order.pnl === undefined || order.pnl === "" ? 0 : Number(order.pnl);
                                    ...
                                    return (
                                        <tr key={order.orderId} className="border-b border-zinc-900 transition-colors last:border-b-0 hover:bg-zinc-900/40">
                                            ...
                                            <td className="px-3 py-2 text-right tabular-nums text-gray-400">{formatQty(order.quantity)}</td>
                                            <td className="px-3 py-2 text-right tabular-nums text-gray-400">{formatQty(order.quantity)}</td>
                                            <td className="px-3 py-2 text-right tabular-nums text-gray-400">{order.leverage}x</td>
                                            <td className="px-3 py-2 text-right tabular-nums text-gray-400">{formatPrice(order.openPrice)}</td>
                                            <td className="px-3 py-2 text-right tabular-nums text-gray-400">{formatPrice(order.closePrice)}</td>
                                            <td className={`px-3 py-2 text-right tabular-nums ${pnlClass}`}>{formatPnl(order.pnl)}</td>
                                            <td className="px-3 py-2 text-right tabular-nums text-gray-400">{formatPrice(order.tp)}</td>
                                            <td className="px-3 py-2 text-right tabular-nums text-gray-400">{formatPrice(order.sl)}</td>
                                            <td className={`px-3 py-2 text-right ${statusClass(order.status)}`}>{order.status}</td>
                                            <td className="px-3 py-2 text-right tabular-nums text-gray-400">{formatDate(order.createdAt)}</td>
                                                ...
                                                {order.status === "RUNNING" && (
                                                    <div className="relative flex items-center justify-end gap-2">
                                                        <button
                                                            ...
                                                            className="p-1 text-gray-500 transition-colors hover:text-gray-200"
